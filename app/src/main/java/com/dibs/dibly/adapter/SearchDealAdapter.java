...
public class SearchDealAdapter extends BaseAdapter {
    ...
    public SearchDealAdapter(Activity activity, List<ObjectDeal> listDeal) {
        this.listDeal = new ArrayList<>();
        this.listDeal.addAll(listDeal);
        this.listTime = new ArrayList<>();
        this.layoutInflater = LayoutInflater.from(activity);
        this.activity = activity;
        options = new DisplayImageOptions.Builder().cacheInMemory(false).showImageForEmptyUri(R.color.white).showImageOnLoading(R.color.white).cacheOnDisc(true).build();
        typeface = Typeface.createFromAsset(activity.getAssets(), "fonts/ProximaNova-Light.otf");
        calculateTime();
    }

    public void setData(List<ObjectDeal> listDeal) {
        this.listDeal.clear();
        this.listDeal.addAll(listDeal);
        calculateTime();
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = this.layoutInflater.inflate(R.layout.row_deal, null);
            ...
            holder.txtDesc_2.setTypeface(typeface);
...
//        ((BaseActivity) activity).overrideFontsLight(holder.lnlRoot);
        ...
        if (outlet != null) {
            holder.txtDesc_2.setText(outlet.getName());
        } else {
            holder.txtDesc_2.setText(listDeal.get(position).getOrganization_name());
        }
        ...
        if (listDeal.get(position).getK_deals_by_outlet() != null) {
            int deals = listDeal.get(position).getK_deals_by_outlet();
            if (deals > 1) {
