 ...
 */
public class OutletDealActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_outlet_deal);

        overrideFontsLight(findViewById(R.id.root));

        if (activityReceiver != null) {
            IntentFilter intentFilter = new IntentFilter();
            intentFilter.addAction(RealTimeService.RECEIVER_GET_DEAL_BY_LIST_ID);
        ...
        if (getIntent().hasExtra("ids") && getIntent().hasExtra("name")) {
            ids = getIntent().getStringExtra("ids");
            outlet_id = getIntent().getStringExtra("outlet_id");
            name = getIntent().getStringExtra("name");

            MyLocation myLocation = MyLocationController.getLastLocation(OutletDealActivity.this);
            if (myLocation != null) {
                Intent intentGetDeal = new Intent(OutletDealActivity.this, RealTimeService.class);
                intentGetDeal.setAction(RealTimeService.ACTION_GET_DEAL_BY_LIST_ID);
                intentGetDeal.putExtra(RealTimeService.EXTRA_LIST_ID_DEAL, ids);
                intentGetDeal.putExtra(RealTimeService.EXTRA_OUTLET_ID, outlet_id);
    ...
    private void initData() {
        ...
        List<ObjectDeal> list = DealController.getDealByDealType(OutletDealActivity.this, 5);
        for (ObjectDeal deal : list) {
            listDeal.add(deal);
        }

        dealAdapter = new DealHomeAdapter(OutletDealActivity.this, listDeal);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.rltBack:
                ...
                break;
            case R.id.rltSearch:
                Intent intentSearch = new Intent(OutletDealActivity.this, SearchActivity.class);
    ...
    public void onDestroy() {
        ...
        DealController.clearDealByDealType(OutletDealActivity.this, 5);
    }
    ...
    private BroadcastReceiver activityReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equalsIgnoreCase(RealTimeService.RECEIVER_GET_DEAL_BY_LIST_ID)) {
